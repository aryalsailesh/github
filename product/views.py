        ...
        if order_qs.exists():
            ...
            if orders.order.filter(product__slug=products.slug).exists():
                ...
                if p.available_quantity > order_items.quantity:
                    ...
                    messages.error(request,'No more items than than in stock.')
                    return redirect('product:cart')
...
def add_to_database(request):
    ...
    for i in cart:
        ...
        #items = OrderItem.objects.create(user=User.objects.get(username='jackdaniels'),ordered=False,product=item['product']['id'])
